"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const azureStorage = require("azure-storage");
const path = require("path");
const vscode = require("vscode");
const infoNode_1 = require("../infoNode");
const fileShareNode_1 = require("./fileShareNode");
class FileShareLabelNode {
    constructor(storageAccount, storageAccountKeys) {
        this.storageAccount = storageAccount;
        this.storageAccountKeys = storageAccountKeys;
    }
    getTreeItem() {
        return {
            label: "[File Shares]",
            collapsibleState: vscode.TreeItemCollapsibleState.Collapsed,
            contextValue: "fileShareLabel",
            iconPath: path.join(__filename, "..", "..", "..", "..", "..", "resources", "AzureFileShare_16x.png"),
        };
    }
    getChildren() {
        const fileService = azureStorage.createFileService(this.storageAccount.name, this.storageAccountKeys[0].value);
        return new Promise((resolve, reject) => {
            fileService.listSharesSegmented(null, (error, result, response) => {
                if (error) {
                    resolve([new infoNode_1.InfoNode(`Failed to list file shares: ${error})`)]);
                }
                const fileshareNodes = result.entries.map((fileShare) => {
                    return new fileShareNode_1.FileShareNode(fileShare, fileService, this);
                });
                resolve(fileshareNodes);
            });
        });
    }
    createFileShare(storageTreeDataProvider) {
        const fileService = azureStorage.createFileService(this.storageAccount.name, this.storageAccountKeys[0].value);
        vscode.window.showInputBox({
            prompt: "Enter file share name",
        }).then((fileShareName) => __awaiter(this, void 0, void 0, function* () {
            if (!fileShareName) {
                return;
            }
            vscode.window.withProgress({
                title: `Creating file share [${fileShareName}] ...`,
                location: vscode.ProgressLocation.Window,
            }, (progress) => __awaiter(this, void 0, void 0, function* () {
                yield new Promise((resolve, reject) => {
                    fileService.createShareIfNotExists(fileShareName, (error, result, response) => {
                        if (error) {
                            vscode.window.showErrorMessage(error.message);
                            reject(error.message);
                        }
                        else {
                            storageTreeDataProvider.refresh(this);
                            resolve();
                        }
                    });
                });
            }));
        }));
    }
}
exports.FileShareLabelNode = FileShareLabelNode;
//# sourceMappingURL=fileShareLabelNode.js.map