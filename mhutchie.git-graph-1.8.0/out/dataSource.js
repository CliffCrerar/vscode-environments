...
class DataSource {
    ...
    commitDetails(repo, commitHash) {
        return new Promise(resolve => {
            Promise.all([
                new Promise((resolve, reject) => this.execGit('show --quiet ' + commitHash + ' --format="' + this.gitCommitDetailsFormat + '"', repo, (err, stdout, stderr) => {
                    if (err) {
    ...
    compareCommits(repo, fromHash, toHash) {
        return new Promise(resolve => {
            ...
            Promise.all(promises)
                .then((results) => resolve({ fileChanges: generateFileChanges(results[0], results[1], toHash === utils_1.UNCOMMITTED ? results[2] : []), error: null }))
                .catch((errorMessage) => resolve({ fileChanges: [], error: errorMessage }));
        });
    }
    getCommitFile(repo, commitHash, filePath, type, diffSide) {
        return (commitHash === utils_1.UNCOMMITTED && type === 'D') || (diffSide === 'old' && type === 'A') || (diffSide === 'new' && type === 'D')
    ...
    pushBranch(repo, branchName, remote, setUpstream) {
    ...
    execDiffTree(repo, cmd, fromHash, toHash) {
        return new Promise((resolve, reject) => this.execGit('-c core.quotepath=false ' + cmd, repo, (err, stdout, stderr) => {
            if (err) {
                reject(getErrorMessage(err, stdout, stderr));
            }
            else {
    ...
    spawnGit(args, repo, successValue) {
        return new Promise((resolve, reject) => {
            let stdout = '', stderr = '', err = false;
            const cmd = cp.spawn(this.gitPath, args, { cwd: repo });
            cmd.stdout.on('data', (d) => { stdout += d; });
...
function getErrorMessage(error, stdout, stderr) {
    let lines;
    if (stdout !== '' || stderr !== '') {
