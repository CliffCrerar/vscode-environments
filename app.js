...
((...params) => {
	...
	argsPassed.includes("-h") && loghelp();

	function loghelp() {
		console.log(`

			'--generate-list': 'Generate a list of extensions in the default folder. alternatively pass a path to look elsewhere for extenesions.',
			'--install-ext-list': 'Takes a list of extensions and installs theses extensions'
		`)
	...
	function installExtensions() {
		const extList = JSON.parse(
			fs.readFileSync(dirToWrite + "/extlist_generated$_Clifford.Crerar_09_Apr_2019-182534.json")
		);
		...
		const dir = 'C:\\Users\\Clifford.Crerar.SURTECH\\.vscode\\extensions'
		//path.join(thisHomedir, ".vscode/extensions");
	...
	function generateExtensionList(homDir) {
		...
		fs.writeFileSync(
			`${dirToWrite}/extlist_generated$_${userName}_${moment().format(
				"DD_MMM_YYYY-HHmmss"
			)}.json`,
