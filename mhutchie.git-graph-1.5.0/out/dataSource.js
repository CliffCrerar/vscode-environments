...
class DataSource {
    ...
    getDiffTreeNameStatus(repo, fromHash, toHash) {
        return new Promise((resolve, reject) => this.execGit('-c core.quotepath=false diff-tree --name-status -r -m --root --find-renames --diff-filter=AMDR ' + fromHash + (fromHash !== toHash ? ' ' + toHash : ''), repo, (err, stdout) => {
            if (err)
                reject();
            else
    ...
    getDiffTreeNumStat(repo, fromHash, toHash) {
        return new Promise((resolve, reject) => this.execGit('-c core.quotepath=false diff-tree --numstat -r -m --root --find-renames --diff-filter=AMDR ' + fromHash + (fromHash !== toHash ? ' ' + toHash : ''), repo, (err, stdout) => {
            if (err)
                reject();
            else
                resolve(stdout.split(eolRegex));
...
function generateFileChanges(nameStatusResults, numStatResults, startAt) {
    let fileChanges = [], fileLookup = {};
