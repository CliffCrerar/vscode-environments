...
class GitGraphView {
        ...
        this.panel.webview.onDidReceiveMessage((msg) => __awaiter(this, void 0, void 0, function* () {
            ...
            switch (msg.command) {
                ...
                case 'viewDiff':
                    this.sendMessage({
                        command: 'viewDiff',
                        success: yield this.viewDiff(msg.repo, msg.fromHash, msg.toHash, msg.oldFilePath, msg.newFilePath, msg.type)
                    });
                    break;
            }
    ...
    viewDiff(repo, fromHash, toHash, oldFilePath, newFilePath, type) {
        let abbrevFromHash = utils_1.abbrevCommit(fromHash), abbrevToHash = utils_1.abbrevCommit(toHash), pathComponents = newFilePath.split('/');
        ...
        return new Promise((resolve) => {
            vscode.commands.executeCommand('vscode.diff', diffDocProvider_1.encodeDiffDocUri(repo, oldFilePath, fromHash === toHash ? fromHash + '^' : fromHash), diffDocProvider_1.encodeDiffDocUri(repo, newFilePath, toHash), title, { preview: true })
                .then(() => resolve(true))
                .then(() => resolve(false));
        });
    }
