...
class MemoryContentProvider {
    ...
    provideTextDocumentContent(uri) {
        return new Promise((resolve, reject) => {
            ...
            const address = query['address'].startsWith('0x') ? parseInt(query['address'].substring(2), 16) : parseInt(query['address'], 10);
            const length = query['length'].startsWith('0x') ? parseInt(query['length'].substring(2), 16) : parseInt(query['length'], 10);
            vscode.debug.activeDebugSession.customRequest('read-memory', { address: address, length: length || 32 }).then((data) => {
                ...
                vscode.window.showErrorMessage(`Unable to read memory from ${utils_1.hexFormat(address, 8)} to ${utils_1.hexFormat(address + length, 8)}`);
