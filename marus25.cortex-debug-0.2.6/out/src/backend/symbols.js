...
class SymbolTable {
    constructor(toolchainPath, executable) {
    ...
    getFunctionByName(name, file) {
        ...
        let matches = this.symbols.filter((s) => s.type === symbols_1.SymbolType.Function && s.scope === symbols_1.SymbolScope.Local && s.name === name && s.file === file);
        if (matches.length !== 0) {
            return matches[0];
        }
        // Fall back to global scope
        matches = this.symbols.filter((s) => s.type === symbols_1.SymbolType.Function && s.scope !== symbols_1.SymbolScope.Local && s.name === name);
