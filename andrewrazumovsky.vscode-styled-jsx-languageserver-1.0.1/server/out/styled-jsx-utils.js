...
var styledJsxPattern = /((<\s*?style\s*?(global)?\s*?jsx\s*?(global)?\s*?>)|(\s*?css\s*?`))/g;
...
function getStyleTagAttributeNames(openingElement) {
    var properties = openingElement.attributes.properties;
    var firstAttribute = properties[0];
    var secondAttribute = properties[1];
    var firstAttributeName = firstAttribute && firstAttribute.name.getText();
    var secondAttributeName = secondAttribute && secondAttribute.name.getText();
    return {
...
function isStyledJsxTag(token) {
    if (token.kind === ts.SyntaxKind.JsxElement) {
        var openingElement = token.openingElement;
        var closingElement = token.closingElement;
        ...
        if (openingElement.tagName.getText() === 'style' && closingElement.tagName.getText() === 'style') {
            var _a = getStyleTagAttributeNames(openingElement), firstAttributeName = _a.firstAttributeName, secondAttributeName = _a.secondAttributeName;
...
function replaceAllWithSpacesExceptCss(textDocument, styledJsxTaggedTemplates, stylesheets) {
    ...
    for (var i = 0; i < styledJsxTaggedTemplates.length; i++) {
        ...
        result += text.slice(styledJsxTaggedTemplates[i].start, styledJsxTaggedTemplates[i].end).replace(expressionPattern, function (str, p1) {
            return p1.replace(/./g, ' ');
        });
...
function getStyledJsxUnderCursor(document, stylesheets, cursorOffset) {
    var styledJsxTaggedTemplates = findStyledJsxTaggedTemplate(document, [cursorOffset]);
