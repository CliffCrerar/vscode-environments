...
function encodeDiffDocUri(repo, path, commit, type) {
    return commit === utils_1.UNCOMMITTED && type !== 'D'
        ? vscode.Uri.file(repo + '/' + path)
        : vscode.Uri.parse(DiffDocProvider.scheme + ':' + utils_1.getPathFromStr(path) + '?commit=' + encodeURIComponent(commit) + '&type=' + type + '&repo=' + encodeURIComponent(repo));
}
exports.encodeDiffDocUri = encodeDiffDocUri;
function decodeDiffDocUri(uri) {
    let queryArgs = decodeUriQueryArgs(uri.query);
    return { filePath: uri.path, commit: queryArgs.commit, type: queryArgs.type, repo: queryArgs.repo };
}
