            ...
            vscode.window.showQuickPick(items, options).then(template => {
                vscode.window
                    ...
                    if (projectName != undefined && projectName != "") {
                        vscode.window
                            .showInputBox({
                            placeHolder: "Optional paramaters: --auth {None, Individual, IndividualB2C, SingleOrg, MultiOrg, Windows}"
                        })
                            .then(optionalParams => {
                            executeCommand_1.executeCommandInOutputChannel([
                                "new",
                                template.label,
                                `-n ${projectName}`,
                                `-o "${folder[0].fsPath}"`,
                                optionalParams
                            ], true, false).stdout.on("end", () => {
                                vscode.commands.executeCommand("vscode.openFolder", vscode.Uri.parse(folder[0].fsPath));
                            });
                        });
                    }
                });
