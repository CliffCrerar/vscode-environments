            ...
            for (const symbol of symbols.filter((x) => x.documentSymbol.kind === vscode_1.SymbolKind.Method)) {
                for (const result of results.values()) {
                    if (result.matches(symbol.parentName, symbol.documentSymbol.name)) {
                        ...
                        if (state) {
                            mapped.push(new testStatusCodeLens_1.TestStatusCodeLens(symbol.documentSymbol.range, state));
                            break;
                        }
                    }
                        ...
                        if (state === utility_1.Utility.codeLensFailed) {
                            mapped.push(new testStatusCodeLens_1.TestStatusCodeLens(symbol.documentSymbol.range, utility_1.Utility.codeLensFailed));
                            break;
                        }
                        else {
