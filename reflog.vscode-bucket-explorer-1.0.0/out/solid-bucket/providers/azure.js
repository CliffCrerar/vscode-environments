...
function getAZURE(options) {
    let blobService = azure.createBlobService(options.accountName, options.accountKey);
    return new AzureWrapper(blobService);
}
exports.getAZURE = getAZURE;
class AzureWrapper {
    ...
    saveObjectIntoBucket(bucketName, filename, object) {
        ...
        return new Promise((resolve, reject) => {
            this.blobService.createBlockBlobFromText(bucketName, filename, JSON.stringify(object), (err, data) => {
                if (!err) {
                    resolve({
                        status: 200,
...
exports.AzureWrapper = AzureWrapper;
